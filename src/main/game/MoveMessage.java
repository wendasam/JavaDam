package main.game;

/**
 * Class ini merepresentasikan pesan yang akan ditampilkan selama proses
 * permainan sedang berjalan
 *
 * @author asus
 */
public enum MoveMessage {

    NOT_DIAGONAL("You can only move diagonally."),
    FORCED_JUMP("Ooops...! You must capture opponent piece first."),
    NO_FREE_SPACE("You can't move onto another piece."),
    ONLY_SINGLE_DIAGONALS("You can only make single moves."),
    NO_BACKWARD_MOVES_FOR_SINGLES("Only kings can move backwards!"),
    NOT_ON_BOARD(""),
    PIECE_BLOCKED("This piece has no diagonal moves."),
    UNKNOWN_INVALID("Not a valid move."),
    SUCCESS("Success");

    private final String name;

    MoveMessage(final String s) {
        name = s;
    }

    @Override
    public final String toString() {
        return this.name;
    }

}
